...
#include "forward_turn_cpp/TurnNode.hpp"
...
tf_listener_(tf_buffer_)
{
  ...
  timer_pos_check_ = create_wall_timer(
    50ms, std::bind(&TurnNode::transform_callback, this));
  timer_publish_ = create_wall_timer(
    50ms, std::bind(&TurnNode::angular_move, this));
}

void
TurnNode::transform_callback()
{
  ...
  if (tf_buffer_.canTransform("odom", "base_footprint", tf2::TimePointZero, &error)) {
    ...
    RCLCPP_INFO(get_logger(), "Angle between odom and base_footprint: %f", angle);
  }
}

void
TurnNode::angular_move()
{
